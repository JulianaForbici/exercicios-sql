...
select id_instrutor, * from instrutor where nome = 'Bernardo Souza';
 
...
select a.nome as atividade, t.horario, t.duracao, i.nome as instrutor
from turma t
left join instrutor i on t.id_instrutor = i.id_instrutor
join atividade a on t.id_atividade = a.id_atividade
where a.nome in ('Ginástica', 'Circo')
order by t.horario;
 
-- 3
...
left join instrutor i on t.id_instrutor = i.id_instrutor
join atividade a on t.id_atividade = a.id_atividade
where t.horario between time '12:00' and time '18:59'
order by t.horario;
...
select act.nome as atividade, t.horario, t.duracao
from turma t
...
select i.nome as instrutor, act.nome as atividade, t.horario, t.duracao
from turma t
join instrutor i on t.id_instrutor = i.id_instrutor
join atividade act on t.id_atividade = act.id_atividade
order by i.nome, act.nome, t.horario;
 
-- 10

select i.nome as instrutor, a.nome as atividade, t.horario, t.duracao
from turma t
left join instrutor i on t.id_instrutor = i.id_instrutor
join atividade a on t.id_atividade = a.id_atividade
order by i.nome nulls last, a.nome, t.horario;
 
-- 11

select nome,
extract(day from dt_nascimento) as dia,
to_char(dt_nascimento, 'tmmonth') as mes
from aluno
where dt_nascimento is not null
and extract(month from dt_nascimento) between 7 and 12
...
select count(*) as total alunos from aluno;

-- 2 

select count(*) as total instrutores from instrutor;
...
-- 11 
select a.id_aluno, a.nome,
count(f.id_frequencia) as presencas
from aluno a
...
order by presencas desc, a.nome;
